...
Defaults the `target` to the host the server was compiled for, with the same reported fallback to the crate's docs.rs-default target on 404 as `search_items`.
